...
1. All characters roll for initiative using their reflexes.
...
When it comes time to begin an encounter each character will roll their *reflexes*, which is their *dexterity* plus their *resolve*. The team with the highest individual iniative goes first. On a tie, the Game Master decides who goes first (usually favouring the player team). Each team will take turns choosing one character to act, and will alternate teams until all characters have acted. Players can choose to take their turns in any order, and may change the order in which they take their turns during each subsequent round. For example: Four players attempt to sneak up on two guards. They fail to be stealthy and alert the guards who begin to attack the players. All six characters will roll for initiative. One of the players rolled the highest initiative so their team will go first. Player #1 will act, then Guard #1, Player #2, Guard #2, Player #3, and finally Player #4. Because there were only two guards, their team did not have any additional characters to take turns between Players #3 and #4.
...
Temporary conditions that alter your primary stats, do not also alter your resource stats. They only change derived values that rely on rolling that primary stat (like skill checks). For example: A character with a strength of r12 is given a temporary effect with alters their size, increasing their strength by +2. This results in a strength of r12+2, which changes their athletics, fortitude, and damage rolls, but it does not change their maximum health from 12 to 14.

In contrast, permanent effects that alter your primary stats do alter your resource stats. For example: If you were given a permanent effect giving you a -2 to your strength, then you would need to reduce your maximum health (as well as your athletics, fortitude, and damage rolls).
...
Powers are broken down into tiers which determine both their difficulty to perform and at what skill rank they are allowed to be learned. All powers of the same tier have the same base focus cost and difficulty. Although this is the primary determining factor, some powers have additional rules allowing you to perform them differently which may modify these values. These modifications only change how the power is performed, and not what tier they belong to.

##### Table: Power Tiers
| Tier | Tier Name | Focus Cost | Difficulty |
|:-:|:-:|:-:|:-:|
| 1 | Cantrip | 1 | 10 (Easy) |
| 2 | Lesser | 2 | 15 (Moderate) |
| 3 | Greater | 3 | 20 (Hard) |
| 4 | Heroic | 4 | 25 (Challenging) |
...
3. Spend focus equal to that power's cost if the result is a graze, success, or critical success. Otherwise, spend only 1 focus.
...
Most powers have varying degrees of success based on how well you rolled the same as normal skill checks: *failure*, *graze*, *success*, and *critical success*. Regardless of your degree of success, you may choose to downgrade the success of your ability (your roll is still the same, but you can choose a lower success category). Failure to perform an ability not only causes it to not perform its intended effect, but may also cause additional unintended effects. The exact nature of these effects are determined by the Game Master based on the ability attempted. The lowest degree of success you can choose to perform is a graze (you cannot choose to downgrade to a failure). <!--If a power does not specify reduced effects as a graze, then grazes are considered failures instead.-->

If the targets of your power choose to resist the effects, they roll the appropriate stat or skill as specified by that power against the results of your skill check for that power. If the targets of that power are granted future opportunities to resist, they will continue to roll resist checks against that same value. Unless a power states otherwise, if it grazes against a resist roll, all effects are negated. Powers that deal damage typically deal reduced damage when grazing against a target's resistance (like attacks) with all other effects being negated.
...
| Stamina | Minimum Reflexes | Passive Reflexes |
...
If the amount of a resource recovered would be greater than the maximum amount you can have of that resource type, it will recover up to your maximum instead. For example: If you have a maximum stamina of 10, but your passive reflexes are 12, then you will only ever recover up to 10 stamina during a rest.
...
A rest occurs when you sleep uninterrupted for approximately eight hours. If a rest is interrupted part way, then it is considered incomplete and only half of the normal resources are recovered instead. The rest may be resumed within two hours to complete the rest and recover the remaining resources. Additionally, any breaks not taken during the day before that rest are also considered to be taken during the rest. Any abilties that can be used a number of times per rest are also restored during a rest, but only after a complete rest. For example: If you took a single break (of the two permitted), then took a rest at the end of the day, you would recover your resources during that rest as if it was combined with the remaining break. If you didn't take any breaks during the day, then it would be as if you took two breaks and rested at the same time.
