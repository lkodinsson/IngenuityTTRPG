...
When using a square grid to track discreet movement, treat every second diagonal space moved as 2m instead of 1m.
...
In addition to the standard defensive actions of [*dodge*](/rules/core_rules/actions#dodge), [*fortify*](/rules/core_rules/actions#fortify), and [*resist*](/rules/core_rules/actions#resist), against attacks you can also *block* and *parry*.
...
If an item you are blocking with would be dealt damage from an attack in excess of its durability, the excess damage is dealt directly to you as though it was from a successful hit. Any effects that apply from being hit are still dealt to you as normal.
...
When you *parry* an attack, choose a wielded item to use for the parry. If that item has a parry value, add it to your physical defense. If it does not, add 1 instead. <!--Parrying allows you to reduce damage to zero instead of the normal minimum of 1.--> After you parry, that item receives 1 damage to its durability.
...
If an item does not have a listed durability and it is made of metal, it has a durability of 10. Otherwise, it has a durability of 4. Items that are broken (have zero durability) cannot be used to block or parry.
...
Damage that characters take is first dealt to their endurance. While at half of their endurance or less, they are considered to be [harmed](./conditions#table-conditions). If damage is dealt in excess of endurance, that damage is instead dealt to health. While at less than their maximum health, they are also considered [bloodied](./conditions#table-conditions).
...
When a character is brought to zero health from damage that would exceed their remaining health and that damage is from a nonlethal source, they do not immediately risk death. Any more damage taken while at zero health, including from a nonlethal source will cause them to risk death as normal.